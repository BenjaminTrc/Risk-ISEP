...
public class Board {
	...
	public void play() {
		...
		while (!victory) {
			...
			// au début du tour on vérifie si le joueur est vainqueur (ex : un autre joueur
			// a détruit le joueur de sa mission)
					...
					if (StdDraw.isMousePressed()) {
						...
						if (territory_id != 0) {
							...
							if (chosen_territory.getOwner() == player_playing) {
								...
								// si c'est un nouveau territoire allié et qu'il y a une armée sélectionné on
								// opère un déplacement
								if (ally_territory != chosen_territory && army_selected == true) {
									...
									// pendant les autres phases, on ne peut déplacer l'unité que sur un territoire
									// voisin et les points de déplacement sont pris en compte
								...
								// l'unité et si le clic n'était pas déjà destiné à un déplacement
								if (game_phase != 2 && players_list.get(player_playing - 1)
										.getArmyPoints() >= selected_unit.getCost() && !action) {
									// on vérifie que l'ajout ne forcera pas le joueur à se retrouver avec un ou
									// plusieurs territoires sans unités
						...
						// si le clic ne correspond à rien en particulier on affiche les données des
						// différents joueurs
						...
						// on garde en tête qu'il y a une armée sélectionné pour un éventuel clic sur un
						// autre territoire
						if (ally_army.size() != 0) {
							army_selected = true;
						}
	...
	public void battle() {
		...
		// Si l'armée est plus grande que 3 on choisit les 3 meilleures unités pour
		// attaquer et les autres unités sont en attente
		int counter = 0;
		...
		/*
		 * Puissance de l'attaque et détermination priorité des unités dans chaque camp attaque
		 */
		...
		// sinon on renvoit les unités attaquantes dans leur territoire originaire et on
		// met à jour les territoires (perte d'unités)
		else {
		...
		for (Region r : regions_list) {
			...
			for (Territory t : territories_list) {
				...
				if (t.getOwner() == player_playing) {
					...
					// s'il n'y a qu'un territoire et qu'il n'y a qu'une unité dans le territoire
					// allié, on ajoute 2 soldats
					...
					// s'il y a plusieurs territoires et moins de 3 unités dans le territoire allié,
					// on ajoute 3 soldats
					...
					// s'il n'y a plus assez de points ou qu'il y a déjà beaucoup d'unités dans le
					// territoire allié
		...
		for (Region r : regions_list) {
			...
			for (Territory t : territories_list) {
				...
				if (t.getOwner() == player_playing && t.getNbUnits() > 2) {
					...
					for (Territory n : neighbour) {
						// si le territoire est ennemi et qu'il y a 2 unités de plus on determine les
						// unités qui vont attaquer
						if (n.getOwner() != player_playing && n.getNbUnits() + 1 < t.getNbUnits()) {
							enemy_territory = n;
							for (int i = 0; i < territory_units.size(); i++) {
								// on vérifie que l'unité peut se déplacer et qu'on laisse une unité dans le
								// territoire initial
	...
	// cette méthode renvoie le nombre de territoire conquis par un joueur, à
	// condition qu'on donne en argument le nombre associé au joueur
	...
	public boolean verifyMission(Mission mission) {
		...
		case 4:
			...
			// Si le nombre de territoires décrit par la mission est atteint, le joueur
			// gagne la partie
	...
	// cette méthode vérifie si un joueur a conquis tous les territoires, si c'est
	...
	// cette méthode permet de gérer l'ajout ou le retrait d'une unité lors des
